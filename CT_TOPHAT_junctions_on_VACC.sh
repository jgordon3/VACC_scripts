...
if [ $GZIP = "gz" ]; then gunzip $PATHTOFASTQ; PATHTOFASTQ=$(echo $PATHTOFASTQ| rev | cut -d. -f2- | rev); else echo "not gzipped"; fi
...
# this only works if files are in CELL_CONDITION_REPLICATE format

CUFFOUT="$WD/$ABRV_NAME/$ABRV_NAME""_cuffout"

# SET ANCHOR LENGTH
if [ -z $ALENGTH ]; then ALENGTH="5"; fi
if [ -z $JUNCTIONS ]; then echo "Junctions were not set"; fi 


#STUFF ACTUALLY GETS DONE HERE

echo "$ABRV_NAME---------TOPHAT-----------------------------------"
tophat -G $GENES -o $ABRV_NAME -a $ALENGTH --no-novel-juncs -j $JUNCTIONS --library-type=$LIBTYPE $GENOME $PATHTOFASTQ

echo "Tophat sent with following parameters: tophat -G $GENES -o $ABRV_NAME -a $ALENGTH --library-type=$LIBTYPE $GENOME $PATHTOFASTQ"
